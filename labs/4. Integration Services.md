...
In this lab we will learn how to create an API gateway, publish a backend service as an API, and apply basic operations and tests using Azure API Management.

> Note
This lab has been lifted from the excellent "Azure Integration Services Workshop" content created by John Billiris, another Microsoft CSA. A link to the full repo is bellow. As well as the APIM content we are using here there are also labs on Logic Apps, Azure Functions, Event Grid and Azure Service Bus.
[Azure Integration Services Workshop](https://github.com/johnbilliris/Azure-Integration-Services-Workshop)
